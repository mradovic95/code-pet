...
Read at `src/app/logger.js:8` and `hooks/scripts/send-event.js:9`. The main-process value is cached at module load and
...
output) are written by `src/app/process-manager.js` regardless. Expect them on user machines even when debug is
disabled.
...
| `CODE_PET_TOOL_START_TTL_MS`       | `600000` (10 min)                                              | Duration-pairing TTL: pending tool-start entries older than this are pruned and never resolve into a `durationMs` (unpaired PreToolUse leak control). Read once at app start in the **Electron main process** — set it where the app is launched, not in the Claude Code session; restart to apply. Invalid or non-positive values fall back to the default. | `src/app/state-machine/pet-context.js`                                                               |
| `CODE_PET_MAX_PENDING_TOOL_STARTS` | `50`                                                           | Cap on the per-pet duration-pairing map; oldest entry evicted when full. Same read-once / main-process / fallback semantics as `CODE_PET_TOOL_START_TTL_MS`.                                                                                                                                                                                                 | `src/app/state-machine/pet-context.js`                                                               |

## 3. `~/.code-pet/marketplace.json`

Defaults come from `src/app/marketplace-constants.js` and are merged with file values in
`src/app/marketplace-config.js`.
...
Written by `src/app/settings-store.js`. These are normally toggled through the Settings window (double-click the pet) —
...
- **Real mode** (default): `MarketplaceAPI` calls the deployed marketplace REST API at `DEFAULT_BASE_URL` (see
  `src/app/marketplace-constants.js`). No configuration required.
...
to `false` and reload the settings window. Backing IPC handlers in `src/app/window-manager.js` stay wired either way;
